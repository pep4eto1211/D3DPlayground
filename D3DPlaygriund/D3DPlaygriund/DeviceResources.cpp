...
void DeviceResources::CreateVertexLayout()
{
	auto context = m_d3dContext.Get();
	auto device = m_d3dDevice.Get();
	auto vertexLayout = m_VertexLayout.Get();
	...
	if (FAILED(CompileShaderFromFile(L"Shader.fx", "VS", "vs_4_0", &pVSBlob)))
	{
		MessageBox(nullptr,
	...
	if (FAILED(device->CreateVertexShader(pVSBlob->GetBufferPointer(), pVSBlob->GetBufferSize(), nullptr, &m_VertexShader)))
	{
		pVSBlob->Release();
		return;
	}
	...
	if (FAILED(device->CreateInputLayout(layout, numElements, pVSBlob->GetBufferPointer(),
	...
	if (FAILED(CompileShaderFromFile(L"Shader.fx", "PS", "ps_4_0", &pPSBlob)))
	{
		MessageBox(nullptr,
			L"The FX file cannot be compiled.  Please run this executable from the directory that contains the FX file.", L"Error", MB_OK);
		return;
	...
	if (FAILED(device->CreatePixelShader(pPSBlob->GetBufferPointer(), pPSBlob->GetBufferSize(), nullptr, &m_PixelShader)))
	{
		pPSBlob->Release();
		return;
	}
...
HRESULT DeviceResources::CompileShaderFromFile(const WCHAR* szFileName, LPCSTR szEntryPoint, LPCSTR szShaderModel, ID3DBlob** ppBlobOut)
{
	HRESULT hr = S_OK;
